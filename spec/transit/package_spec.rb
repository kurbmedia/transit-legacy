...
describe 'a Transit::Package' do
  ...
  its(:ancestors){ should include(Mongoid::Document) }
  it 'stores in the default collection' do
    subject.collection.name.should == "posts"
  end
  
  describe 'a subclass' do
    subject{ Article } 
    it 'stores in the parent collection' do
      subject.collection.name.should == "posts"
    end
  end
